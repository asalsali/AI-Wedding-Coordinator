  ...
  return (
    <div style={{ width: '100%', maxWidth: 420 }}>
      <h1 className="wf-serif" style={{ fontSize: 40, color: 'var(--wf-forest)', margin: '0 0 10px', fontWeight: 600, letterSpacing: '-0.02em' }}>
        Welcome <em style={{ fontWeight: 500 }}>back.</em>
      </h1>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <AuthField label="Email address" id="signin-email">
          <input id="signin-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" className="wf-sans" style={fieldInputStyle} onFocus={focusStyle} onBlur={blurStyle} />
        </AuthField>

        <AuthField label="Password" id="signin-password">
          <div style={{ position: 'relative' }}>
            <input id="signin-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" className="wf-sans" style={{ ...fieldInputStyle, paddingRight: 44 }} onFocus={focusStyle} onBlur={blurStyle} />
      ...
      <p className="wf-sans" style={{ textAlign: 'center', marginTop: 28, fontSize: 13, color: 'var(--wf-ink-60)' }}>
        New to Wedflow?{' '}
        <Link href="/sign-up" style={{ color: 'var(--wf-forest)', fontWeight: 600, textDecoration: 'none' }}>Create account</Link>
      </p>
    </div>
  )
}
